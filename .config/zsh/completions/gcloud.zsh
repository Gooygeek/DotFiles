if command -v gcloud >/dev/null 2>&1; then
    # `gcloud info` method has fewer dependencies, but is slow
    ...
    if [[ -e "$(dirname $(readlink -f $(which gcloud)))/../path.zsh.inc" ]] ; then
        source "$(dirname $(readlink -f $(which gcloud)))/../path.zsh.inc"
    else
        # echo "gcloud path.zsh.inc not found, skipping gcloud path completions" >&2
    fi
    if [[ -e "$(dirname $(readlink -f $(which gcloud)))/../completion.zsh.inc" ]] ; then
        source "$(dirname $(readlink -f $(which gcloud)))/../completion.zsh.inc"
    else
        # echo "gcloud completion.zsh.inc not found, falling back to vendored completion script" >&2


        # original gcloud completion script
